...
// 1. IMPORTAMOS LAS BASES DE DATOS CON TUS NOMBRES EXACTOS DE ARCHIVO
...
// 2. CREAMOS EL MOTOR DE BÚSQUEDA
...
const EstrellasFondo = () => (
  <div
    ...
    style={{
      backgroundImage:
        'url("https://images.unsplash.com/photo-1506318137071-a8e063b4bec0?q=80&w=3000&auto=format&fit=crop")',
...
export default function App() {
  ...
  // 3. LA MAGIA: LEER LOS DATOS GIGANTES
  const obtenerVersiculos = () => {
    try {
      const biblia = BIBLIA_VERSIONES[versionActual];
      const libroData = biblia.books.find((b) => b.name === libroActual);
      if (!libroData)
        return [{ numero: '', texto: 'Libro no encontrado en esta versión.' }];

      const capituloData = libroData.chapters[capituloActual - 1];
      if (!capituloData)
        return [{ numero: '', texto: 'Capítulo no disponible.' }];

      // Filtramos solo los versículos reales y sacamos su número
      ...
      return versiculos.length > 0
        ? versiculos
        : [{ numero: '', texto: 'No hay texto para este capítulo.' }];
    } catch (e) {
  ...
  const cambiarCapitulo = (direccion) => {
    try {
      const libroData = BIBLIA_VERSIONES[versionActual].books.find(
        (b) => b.name === libroActual
      );
      const totalCapitulos = libroData.chapters.length;
  ...
  const librosAntiguo = LIBROS_MENU.filter(
    (l) => l.testamento === 'Antiguo Testamento'
  );
  const librosNuevo = LIBROS_MENU.filter(
    (l) => l.testamento === 'Nuevo Testamento'
  );
      ...
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500 select-none relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center gap-10 w-full max-w-sm">
          <img
            ...
            alt="Logo CyM Biblia"
            className="w-80 h-80 object-contain drop-shadow-[0_0_40px_rgba(204,163,0,0.6)] hover:scale-105 transition-transform duration-500"
          />
          <div className="space-y-4 mt-4">
            <h1 className="text-5xl font-black text-[#fcd34d] tracking-widest drop-shadow-md">
              CyM{' '}
              <span className="font-light opacity-80 text-white">BIBLIA</span>
            </h1>
            <p className="text-xs font-black text-[#cca300] uppercase tracking-[0.3em] opacity-80 drop-shadow-md">
              LEÉ, CRECÉ Y MUTIPLICÁ
            </p>
          </div>
          <button
            onClick={() => setMostrarPortada(false)}
            className="group w-full mt-8 bg-gradient-to-r from-[#ffd700] to-[#b8860b] text-black py-4 rounded-full font-black text-xs flex items-center justify-center gap-3 shadow-2xl shadow-[#cca300]/40 hover:scale-105 transition-all duration-300 uppercase tracking-wider"
          >
            Comenzar Lectura{' '}
            <LogIn
              size={18}
  ...
  return (
    <div
      className={`min-h-screen flex flex-col transition-colors duration-500 font-serif relative ${
        themeStyles[tema].split(' ')[0]
      } ${themeStyles[tema].split(' ')[1]}`}
    >
      {tema === 'cym' && <EstrellasFondo />}

      <nav
        className={`sticky top-0 z-50 px-2 md:px-6 py-3 shadow-md flex items-center justify-between backdrop-blur-md border-b ${navStyles[tema]}`}
      >
        <div
        ...
        >
          ...
          <img
            src="https://i.postimg.cc/3RzYnbnB/image-11-png.png"
            alt="Logo CyM"
            className="w-10 h-10 md:w-16 md:h-16 object-contain drop-shadow-[0_0_12px_rgba(204,163,0,0.5)]"
          />
          <h1 className="text-lg md:text-2xl font-black tracking-wider hidden sm:block">
            CyM <span className="font-light opacity-80">Biblia</span>
          </h1>
        </div>
          ...
          {vistaActual === 'lector' && (
            <>
              <select
                ...
                className="bg-[#cca300]/20 rounded-full px-2 py-1 font-black text-[#fcd34d] text-xs md:text-sm outline-none cursor-pointer appearance-none text-center border border-[#cca300]/30 mr-1 md:mr-2"
              >
                <option value="RVR1960" className="text-black">
                  RV1960
                </option>
                <option value="NTV" className="text-black">
                  NTV
                </option>
                <option value="DHH" className="text-black">
                  DHH
                </option>
                <option value="LBLA" className="text-black">
                  LBLA
                </option>
                <option value="TLA" className="text-black">
                  TLA
                </option>
              </select>
              ...
              <select
                value={libroActual}
                onChange={(e) => {
                  setLibroActual(e.target.value);
                  setCapituloActual(1);
                }}
                className="bg-transparent font-bold text-xs md:text-base outline-none cursor-pointer appearance-none text-right max-w-[70px] md:max-w-[100px] truncate"
              >
                {LIBROS_MENU.map((l) => (
                  <option
                    key={l.nombre}
              ...
              {/* SELECTOR DINÁMICO DE CAPÍTULOS */}
              {(() => {
                ...
                try {
                  ...
                  if (libroData && libroData.chapters)
                    cantidadCapitulos = libroData.chapters.length;
                } catch (e) {}

                return (
                  <select
                    value={capituloActual}
                    onChange={(e) => setCapituloActual(Number(e.target.value))}
                    className="bg-transparent font-bold text-xs md:text-base outline-none cursor-pointer appearance-none"
                  >
                    {Array.from(
                      { length: cantidadCapitulos },
                      (_, i) => i + 1
                    ).map((num) => (
      ...
      {mostrarAjustes && (
        <div
          className={`fixed top-20 right-6 p-5 rounded-2xl shadow-2xl border w-72 z-40 animate-in fade-in slide-in-from-top-4 ${
            tema === 'cym'
              ? 'bg-[#141414] border-[#cca300]/50'
              : 'bg-white border-slate-200'
          }`}
        >
          <p className="text-[10px] font-black uppercase tracking-widest mb-3 opacity-50">
            Tamaño de lectura
          </p>
          <div className="flex items-center justify-between mb-6 bg-black/5 rounded-lg p-1 border border-current/10">
            <button
              onClick={() => setTamañoFuente(Math.max(14, tamañoFuente - 2))}
            ...
            <button
              onClick={() => setTamañoFuente(Math.min(32, tamañoFuente + 2))}
              className="p-2 hover:bg-black/10 rounded"
            >
              <Type size={22} />
            </button>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 p-3 rounded-xl border-2 flex justify-center shadow-sm ${
                tema === 'claro'
            ...
            <button
              ...
              className={`flex-1 p-3 rounded-xl border-2 flex justify-center shadow-sm ${
                tema === 'sepia'
                  ? 'border-[#8b6b4a] bg-[#e6d5b8]'
                  : 'border-transparent bg-[#fbf0d9] text-[#5f4b32]'
              }`}
            >
              <BookOpen size={18} />
            </button>
            <button
              ...
              className={`flex-1 p-3 rounded-xl border-2 flex justify-center shadow-sm ${
                tema === 'cym'
                  ...
                  : 'border-transparent bg-[#0a0a0a] text-[#ffd700]'
              }`}
            >
              <Sparkles size={18} />
            </button>
      ...
      <main className="flex-grow max-w-3xl mx-auto w-full px-6 py-8 relative z-10">
        {vistaActual === 'home' && (
          <div className="animate-in fade-in duration-500">
            <div
              onClick={() => abrirLibro('Salmos', 23)}
              className="relative overflow-hidden rounded-3xl p-8 mb-10 cursor-pointer group shadow-xl transition-transform hover:scale-[1.02] border border-[#cca300]/40 backdrop-blur-md"
              style={{
                background:
            ...
            >
              ...
              <p className="text-[#cca300] font-black text-[10px] uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                <Sparkles size={12} /> Lectura Diaria
              </p>
              <h2 className="text-3xl font-black text-[#fcd34d] mb-3">
                Salmos 23
              </h2>
              <p className="text-slate-300 text-base md:text-lg font-medium italic line-clamp-2 pr-8">
            ...
            <div className="space-y-8">
              <div>
                <h3 className="text-[11px] font-black text-[#cca300] uppercase tracking-[0.2em] mb-3 border-b border-[#cca300]/20 pb-2 bg-black/60 p-2 rounded backdrop-blur-sm inline-block">
                  Antiguo Testamento
                </h3>
                <div className="flex flex-col gap-2">
                  {librosAntiguo.map((libro) => (
                    <button
                      key={libro.nombre}
                      onClick={() => abrirLibro(libro.nombre, 1)}
                      className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all border ${
                        tema === 'cym'
                          ? 'bg-black/60 backdrop-blur-md border-[#cca300]/20 hover:border-[#cca300]/60 hover:bg-[#cca300]/10'
          ...
          <div className="animate-in slide-in-from-right-8 duration-300 bg-black/70 p-6 md:p-10 rounded-3xl backdrop-blur-md border border-[#cca300]/20 shadow-2xl">
            <h2
              className={`text-3xl font-black mb-12 text-center ${
                tema === 'cym' ? 'text-[#ffd700]' : ''
              }`}
              style={{ fontSize: `${tamañoFuente * 2.2}px` }}
            >
            ...
            <div
              ...
              style={{ fontSize: `${tamañoFuente}px`, lineHeight: '1.9' }}
            >
              {versiculosActuales.map((versiculo, index) => (
                <p key={index} className="relative group cursor-text">
                  <sup
                    className={`absolute -left-6 top-1.5 text-[0.6em] font-black select-none ${
                      tema === 'cym' ? 'text-[#ffd700]/60' : 'opacity-40'
                    }`}
                  >
                    {versiculo.numero}
                  </sup>
                  <span
                    className={`rounded p-0.5 transition-colors duration-300 ${
                      tema === 'cym'
      ...
      <footer
        className={`mt-auto p-4 flex justify-between items-center backdrop-blur-md border-t relative z-10 ${navStyles[tema]}`}
      >
        {vistaActual === 'lector' ? (
          <>
            <button
              onClick={() => cambiarCapitulo(-1)}
              className="flex items-center gap-2 font-bold opacity-60 hover:opacity-100 transition-opacity text-sm"
            >
              <ChevronLeft size={18} /> Anterior
            </button>
            <span className="text-[10px] font-black tracking-widest uppercase opacity-40 hidden sm:block">
              CyM Biblia v1.0
            </span>
            <button
              onClick={() => cambiarCapitulo(1)}
          ...
          <div className="w-full text-center">
            <span className="text-[10px] font-black tracking-widest uppercase opacity-40">
              Desarrollado por Max para Crecer y Multiplicar
            </span>
          </div>
        )}
      </footer>
